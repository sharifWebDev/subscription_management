                        ...
                        <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
                            <div
                                ...
                                <div class="align-items-center">
                                    ...
                                    <p class="text-muted mb-0 small mt-1">Choose your payment method and fill in the details</p>
                                </div>
                                <!-- Billing Cycle -->
                                ...
                                <form action="javascript:void(0);" id="checkoutForm">
                                    ...
                                    <div class="row g-3 mb-4">
                                        ...
                                        <div class="col-6 col-md-6 col-lg-4">
                                            ...
                                            <input type="tel" value="{{ old('phone', auth()->user()?->phone ?? '') }}" class="form-control form-control-md rounded-3"
        ...
        $(document).ready(function() {
              ...
              function fetchUserPaymentMethods() {
                axios.get('/payment-methods')
                    .then(response => {
                        // The response should contain the saved methods with gateway_payment_method_id
            ...
            function renderSavedPaymentMethods() {
                ...
                userPaymentMethods.forEach(method => {
                    ...
                    // Use gateway_payment_method_id for Stripe, not the local ID
                    ...
                    let icon = method.brand === 'visa' ? 'fab fa-cc-visa' :
                            method.brand === 'mastercard' ? 'fab fa-cc-mastercard' :
                            method.brand === 'amex' ? 'fab fa-cc-amex' : 'fas fa-credit-card';

                    html += `
                        <div class="col-md-6 col-lg-4 col-sm-6">
                            <div class="card saved-method-card ${isDefault} ${selected} p-3"
                                onclick="selectSavedMethod('${gatewayMethodId}', '${method.id}', this)">
            ...
            window.selectSavedMethod = function(gatewayMethodId, localMethodId, element) {
                // Find method by either gateway ID or local ID
                ...
                // Send the gateway payment method ID (pm_xxx), not the local ID
                $('#selected_gateway').val(selectedGateway);
                $('#payment_method_id').val(gatewayMethodId); // This is the important part!
                $('#paymentMethodsSection').hide();
                ...
                function renderBillingCycles() {
                    ...
                    planData.prices.forEach(price => {
                        ...
                        html += `
                        <span
                                class="alert w-100 text-center py-1 ${isSelected ? 'alert-primary' : 'alert-outline-primary'} billing-cycle"
                                data-price-id="${price.id}"
                                data-amount="${price.amount}"
                                data-interval="${price.interval}">
                            ${ucfirst(price.interval)}ly
                            ${price.interval === 'year' ? '<span class="badge bg-success ms-1">Save 20%</span>' : ''}
                        </span>
                    `;
                    });
...
SWIFT Code: ${bankDetails.swift_code}`;
        ...
        async function processGuestCheckout() {
            ...
            try {
                ...
                $('#submitBtn').prop('disabled', false).html('<i class="fas fa-paper-plane me-2"></i>Complete Purchase');
            }
        }
        ...
        $('#verifyOtpBtn').on('click', async function() {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    if (response.data.data?.requires_redirect) {
                        setTimeout(() => {
            ...
            try {
                ...
                if (response.data.success) {
                    if (response.data.data?.redirect_url) {
                        window.location.href = response.data.data.redirect_url;
                    } else if (response.data.data?.requires_action && response.data.data.client_secret) {
                        toastr.info('Please complete the 3D Secure authentication...');
                        ...
                        if (error) {
                            ...
                            try {
                                const confirmResponse = await axios.post('/checkout/confirm-stripe', {
