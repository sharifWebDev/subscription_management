...
$(document).ready(function() {
    ...
    // Load both APIs
    Promise.all([
        $.ajax({ url: '/api/v1/usage-stats', method: 'GET' }),
        $.ajax({ url: '/api/v1/usage-forecast', method: 'GET' })
    ...
    function updateSummaryCards() {
        ...
        $('#total-crud-usage').text(crudFeature ? crudFeature.total_usage + ' / ' + crudFeature.total_limit : '0');
        $('#total-api-usage').text(apiFeature ? apiFeature.total_usage + ' / ' + apiFeature.total_limit : '0');
        $('#total-storage-usage').text(storageFeature ? storageFeature.total_usage + ' GB / ' + storageFeature.total_limit + ' GB' : '0 GB');
    }
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                const percentage = (feature.total_usage / feature.total_limit * 100).toFixed(1);
    ...
    function updateSubscriptionTable() {
        ...
        const crudSubscriptions = usageData.detailed_summaries.filter(s => s.feature_code === 'crud_generation');
        const forecastSubscriptions = forecastData.forecasts_by_feature?.crud_generation?.subscriptions || [];

        let html = '';

        crudSubscriptions.forEach(sub => {
            const forecast = forecastSubscriptions.find(f => f.subscription_id === sub.subscription_id) || {};
            ...
            html += `<div class="progress-bar bg-${statusClass}" style="width: ${percentage}%"></div>`;
            html += '</div>';
            html += `<small>${percentage}%</small>`;
            html += '</td>';
            html += `<td>${sub.remaining}</td>`;
            html += `<td>${forecast.projected_usage ? forecast.projected_usage.toFixed(2) : '0'}</td>`;
            html += `<td><span class="badge bg-${statusClass}">${statusText}</span></td>`;
            html += '</tr>';
        });

        if (html === '') {
            html = '<tr><td colspan="9" class="text-center text-muted">No subscription data available</td></tr>';
        }
    ...
    function updateForecastDetails() {
        ...
        Object.values(forecastData.forecasts_by_feature).forEach(feature => {
            ...
            html += `<div class="progress-bar ${willExceed ? 'bg-warning' : 'bg-success'}" style="width: ${Math.min(progress, 100)}%"></div>`;
            if (feature.total_projected > feature.total_limit) {
                const overagePercent = Math.min((feature.total_projected / feature.total_limit * 100) - 100, 100);
                html += `<div class="progress-bar bg-danger" style="width: ${overagePercent}%"></div>`;
            }
            html += '</div>';
    ...
    function showError() {
        ...
        $('table tbody').html('<tr><td colspan="9" class="text-center text-danger">Failed to load data. Please refresh the page.</td></tr>');
