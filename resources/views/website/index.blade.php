...
<script>
    $(document).ready(function() {
        ...
        function fetchPlans() {
            ...
            .then(response => {
                allFeatures = response.data.data || [];
                renderPlans();
        ...
        function renderPlans() {
            ...
            plans.forEach(plan => {
                let monthlyPrice = plan.prices?.find(p => p.interval === 'month');
                let yearlyPrice = plan.prices?.find(p => p.interval === 'year');
                ...
                if (priceToShow) {
                    if (priceToShow.amount > 0) {
                        priceDisplay = priceToShow.amount_with_currency || '$' + parseFloat(priceToShow.amount).toFixed(2);
                        periodDisplay = '/ ' + (priceToShow.interval_description || priceToShow.interval);
                    } else {
                        priceDisplay = 'Free';
                        periodDisplay = '';
                    }
                ...
                let featuresHtml = '';
                let displayedFeatures = plan.features?.slice(0, 5) || [];

                displayedFeatures.forEach(feature => {
                    let badge = feature.value === 'unlimited' ? '<span class="badge bg-info ms-1">Unlimited</span>' : '';
        ...
        function renderComparisonTable() {
            if (plans.length === 0 || allFeatures.length === 0) return;

            // Render header
            let headerHtml = '<th style="width: 200px;">Feature</th>';
            plans.forEach(plan => {
                headerHtml += `<th class="text-center">${plan.name}</th>`;
            });
            ...
            allFeatures.forEach(feature => {
                ...
                plans.forEach(plan => {
                    let planFeature = plan.features?.find(f => f.feature_id === feature.id);
