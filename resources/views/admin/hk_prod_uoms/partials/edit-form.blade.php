            ...
            <form method="POST" id="edithk_prod_uomsForm" action="{{ url('api/v1/hk-prod-uoms/update/' . request()->id ?? "") }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                ...
                <div class="mb-3 col-12 col-md-4 col-lg-3">
                    <div class="form-group">
                        ...
                        <input type="number" name="sequence" min="0" class="form-control @error('sequence') is-invalid @enderror" value="{{ old('sequence', $query->sequence ?? "") }}" placeholder="Enter Sequence..." id="edit_sequence" required>
                        @error('sequence')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
