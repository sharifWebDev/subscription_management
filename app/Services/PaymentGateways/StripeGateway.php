...
public function createPaymentIntentWithSavedMethod(array $data, string $paymentMethodId): array
{
    try {
        ...
        $paymentIntent = PaymentIntent::create([
            ...
            'off_session' => false,
            'return_url' => config('app.url') . '/dashboard/subscriptions',
...
public function createPaymentIntent(array $data): array
{
    try {
        ...
        $paymentIntentData = [
            ...
            'description' => $data['description'] ?? 'Subscription Payment',
            'return_url' => $data['return_url'] ?? config('app.url') . '/api/v1/checkout/callback/stripe',
            'metadata' => [
                'user_id' => $data['user_id'] ?? null,
                'payment_master_id' => $data['payment_master_id'] ?? null,
        ...
        // Option 1: Use automatic_payment_methods with redirects disabled
        $paymentIntentData['automatic_payment_methods'] = [
            'enabled' => true,
            'allow_redirects' => 'never', // This prevents redirect-based payment methods
        ];

        // Option 2: Specify payment method types explicitly (cards only)
...
public function confirmPaymentIntent(string $paymentIntentId, ?string $paymentMethodId = null): array
{
    try {
        ...
        if ($paymentIntent->status === 'requires_confirmation' ||
            ...
            // Add return_url for redirect-based payment methods
            $confirmParams['return_url'] = config('app.url') . '/api/v1/checkout/callback/stripe';
